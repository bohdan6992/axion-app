...
namespace TradingBridgeApi.Services.Strategy.OpenDoor;

public sealed class OpenDoorFilesService
{
    private static readonly TimeSpan RefreshTtl = TimeSpan.FromDays(5);

    private readonly ISignalsSource _src;
    private readonly ILogger<OpenDoorFilesService> _log;
    ...
    private const string StrategyCode = "opendoor";

    // fallback dir if manifest missing
    private const string FallbackStrategyDir = "opendoor";
    ...
    public OpenDoorFilesService(ISignalsSource src, ILogger<OpenDoorFilesService> log)
    {
        _src = src;
        _log = log;
    }
